...
public interface OrderMapper {
    @Insert("insert into orders(order_no,user_id,total_amount,status,payment_status,expire_at,created_at,updated_at) " +
            "values(#{orderNo},#{userId},#{totalAmount},#{status},#{paymentStatus},#{expireAt},now(),now())")
